...
from mlp_model_relu import MNIST_MLP_RELU
from mlp_model_Sig import MNIST_MLP_SIG
from mlp_model_tanh import MNIST_MLP_TANH
...
def main():
    ...
    # The model
    model1 = MNIST_MLP_RELU(layer_sizes=[784, 10])
    model1.set_device(device)
    
    #model 2
    model2 = MNIST_MLP_SIG(layer_sizes=[784, 10])
    model2.set_device(device)
    
    # model 3
    model3 = MNIST_MLP_TANH(layer_sizes=[784, 10])
    ...
    plt.plot(vah1, label='ReLU')
    plt.plot(vah2, label='Sigmoid')
    plt.plot(vah3, label='Tanh')
    plt.legend()
    plt.ylabel('Accuracy')
    plt.xlabel('Epochs')
    plt.tight_layout()
